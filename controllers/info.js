...
class InfoController extends Telegram.TelegramBaseController {
  ...
  cmHandler($) {
    ...
    if (msg.length == 1) cm = msg[0].toUpperCase();
      ...
      for (var i = 1; i < msg.length; i++) cm += '-' + msg[i].toUpperCase();
    }

    if (!cm)
      return $.sendMessage('`/cm <cryptocurrency name>`', {
        parse_mode: 'Markdown'
      });
    else {
      var coinDictionary = {};
      ...
      request.get(
        ...
        function(err, res, data) {
          ...
          request.get(
            'https://api.coinmarketcap.com/v1/ticker/' + cm + ' /',
            function(err, res, body) {
              var obj = JSON.parse(body);
              if (obj.error)
                return $.sendMessage("`'" + msg + "' doesn't exist!`", {
                  parse_mode: 'Markdown'
                });
              else {
                var p1 = obj[0].percent_change_1h,
                  p24 = obj[0].percent_change_24h,
                  s1,
                  s24;
                ...
                var str =
                  ...
                  ')\n' +
                  ' $' +
                  add_coma_to_number(obj[0].price_usd) +
                  '  Ƀ' +
                  obj[0].price_btc +
                  '  1h:' +
                  p1 +
                  ...
                  '\n' +
                  ' MrktCap: ' +
                  add_coma_to_number(obj[0].market_cap_usd) +
                  '\n' +
                  ' Vol24h: ' +
                  add_coma_to_number(obj[0]['24h_volume_usd']) +
                  '\n' +
                  ' CrcSupply: ' +
                  add_coma_to_number(obj[0].available_supply) +
                  '```';
  ...
  // convert amount of currency to other currency
  convertHandler($) {
  ...
  // full market info
  fcmHandler($) {
    request.get('https://api.coinmarketcap.com/v1/global/', function(
      err,
      res,
      body
    ) {
      ...
      var str =
        '``` Total Market Cap: $' +
        add_coma_to_number(obj.total_market_cap_usd.toString()) +
        '\n' +
        ' Total 24h Volume: $' +
        add_coma_to_number(obj.total_24h_volume_usd.toString()) +
        '\n' +
        ' BTC Porcentage: ' +
        obj.bitcoin_percentage_of_market_cap +
        '% ' +
        '```';
      $.sendMessage(str, { parse_mode: 'Markdown' });
    });
...
// add comas to numbers and decimal point
var add_coma_to_number = function(num) {
  if (num != null) {
